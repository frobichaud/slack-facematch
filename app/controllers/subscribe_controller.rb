class SubscribeController < ApplicationController
  def create
    # When registering the app on Slack, the server must reply with the challenge parameter
    event = params[:event]
    if params[:challenge].present?
      return render json: {challenge: params[:challenge]}
    elsif event.nil?
      return render json: {}, status: 401
    end
    
    # TODO: verify Slack secret

    # Skip messages that are generated by this bot
    unless event[:bot_id] && event[:type] == 'message'
      case event[:text]
      when 'play'
        play event
      when 'help'
        FaceMatch::SlackHelper.help event[:channel]
      end
    end

    render json: {}
  end

  private
  def play(event)
    profiles = FaceMatch::SlackHelper.fetch_profiles(event[:user])
    FaceMatch::SlackHelper.send_quiz_message(profiles, event[:channel])
  end
end
